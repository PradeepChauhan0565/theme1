...
class MetalPurityComponent extends Component
{
    use WithFileUploads;
    use WithPagination;
    ...
    public function update()
    {
        ...
        MetalPurity::find($this->sz_id)->update([
            'code' => $this->code,
            'description' => $this->description,
            'order_by' => $this->order_by ?: null,
    ...
    {
        MetalPurity::find($this->sz_id)->delete();
        $this->sz_id = null;
        $this->delete = null;
    }
    ...
    public function render()
    {
        ...
        $this->status = [];
        $datas = MetalPurity::all();
        foreach ($datas as $datas) {
            $this->status[$datas->id] = $datas->status;
        }

        $table = new MetalPurity();
        $columns = $table->getTableColumns('metal_purities');
        return view('livewire.metal-purity-component', [
            'metal_purities' => MetalPurity::when($this->search, function ($q) use ($columns) {
