...
class ColorStoneShapeComponent extends Component
{
    use WithFileUploads;
    use WithPagination;
    ...
    public function update()
    {
        ...
        ColorStoneShape::find($this->sz_id)->update([
            'code' => $this->code,
            'description' => $this->description,
            'order_by' => $this->order_by ?: null,
    ...
    {
        ColorStoneShape::find($this->sz_id)->delete();
        $this->sz_id = null;
        $this->delete = null;
    }
    ...
    public function statusChange($id)
    {
        ...
        ColorStoneShape::find($id)->update([
            'status' => $d,
        ]);
    }
    ...
    public function render()
    {
        ...
        $this->status = [];
        $datas = ColorStoneShape::all();
        foreach ($datas as $datas) {
            $this->status[$datas->id] = $datas->status;
        }

        $table = new ColorStoneShape();
        $columns = $table->getTableColumns('color_stone_shapes');
        return view('livewire.color-stone-shape-component', [
            'color_stone_shapes' => ColorStoneShape::when($this->search, function ($q) use ($columns) {
