...
class MetalColorComponent extends Component
{
    use WithFileUploads;
    use WithPagination;
    ...
    public function update()
    {
        ...
        MetalColor::find($this->mt_id)->update([
            'code' => $this->code,
            'description' => $this->description,
            'order_by' => $this->order_by ?: null,
    ...
    {
        MetalColor::find($this->mt_id)->delete();
        $this->mt_id = null;
        $this->delete = null;
    }
    ...
    public function render()
    {
        ...
        $this->status = [];
        $datas = MetalColor::all();
        foreach ($datas as $datas) {
            $this->status[$datas->id] = $datas->status;
        }

        $table = new MetalColor();
        $columns = $table->getTableColumns('metal_types');
        return view('livewire.metal-color-component', [
            'metalColors' => MetalColor::when($this->search, function ($q) use ($columns) {
