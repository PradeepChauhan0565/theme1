...
class DiamondSizeComponent extends Component
{
    use WithFileUploads;
    use WithPagination;
    ...
    public function update()
    {
        ...
        DiamondSize::find($this->mt_id)->update([
            'code' => $this->code,
            'description' => $this->description,
            'order_by' => $this->order_by ?: null,
    ...
    {
        DiamondSize::find($this->mt_id)->delete();
        $this->mt_id = null;
        $this->delete = null;
    }
    ...
    public function render()
    {
        ...
        $this->status = [];
        $datas = DiamondSize::all();
        foreach ($datas as $datas) {
            $this->status[$datas->id] = $datas->status;
        }

        $table = new DiamondSize();
        $columns = $table->getTableColumns('diamond_sizes');
        return view('livewire.diamond-size-component', [
            'diamondSizes' => DiamondSize::when($this->search, function ($q) use ($columns) {
