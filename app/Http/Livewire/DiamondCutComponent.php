...
class DiamondCutComponent extends Component
{
    use WithFileUploads;
    use WithPagination;
    ...
    public function update()
    {
        ...
        DiamondCut::find($this->sz_id)->update([
            'code' => $this->code,
            'description' => $this->description,
            'order_by' => $this->order_by ?: null,
    ...
    {
        DiamondCut::find($this->sz_id)->delete();
        $this->sz_id = null;
        $this->delete = null;
    }
    ...
    public function render()
    {
        ...
        $this->status = [];
        $datas = DiamondCut::all();
        foreach ($datas as $datas) {
            $this->status[$datas->id] = $datas->status;
        }

        $table = new DiamondCut();
        $columns = $table->getTableColumns('diamond_cuts');
        return view('livewire.diamond-cut-component', [
            'diamond_cuts' => DiamondCut::when($this->search, function ($q) use ($columns) {
