...
class ColorStoneNameComponent extends Component
{
    use WithFileUploads;
    use WithPagination;
    ...
    public function update()
    {
        ...
        ColorStoneName::find($this->mt_id)->update([
            'code' => $this->code,
            'description' => $this->description,
            'order_by' => $this->order_by ?: null,
    ...
    {
        ColorStoneName::find($this->mt_id)->delete();
        $this->mt_id = null;
        $this->delete = null;
    }
    ...
    public function render()
    {
        ...
        $this->status = [];
        $datas = ColorStoneName::all();
        foreach ($datas as $datas) {
            $this->status[$datas->id] = $datas->status;
        }

        $table = new ColorStoneName();
        $columns = $table->getTableColumns('color_stone_names');
        return view('livewire.color-stone-name-component', [
            'colorStoneNames' => ColorStoneName::when($this->search, function ($q) use ($columns) {
